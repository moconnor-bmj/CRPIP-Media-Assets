        ...
        if (opts.tagsContainer !== null) {
          ...
          if (tlid.length > 1) {
            if (typeAheadMess) {
              ...
              var tagWrapperObj =$self.siblings('.tagWrapper');
              lastTagObj = tagWrapperObj.find("#" + $self.data("tm_rndid") + "_" + tlid[tlid.length - 2]);
              //lastTagObj = $self.siblings("#" + $self.data("tm_rndid") + "_" + tlid[tlid.length - 2]);
              lastTagObj.after($el);
              tagWrapperObj.scrollTop(tagWrapperObj[0].scrollHeight);
            }
          } else {
            if (typeAheadMess) {
